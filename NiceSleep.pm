...
# $Id: NiceSleep.pm,v 1.10 2004/11/09 19:42:31 joshr Exp $
#############################################################################
use 5.004;	# tested this far back and up to 5.8.4
use strict;	# please
...
use vars qw ( $_sleepfactor $_minruntime $_minsleeptime $_maxload 
		...
		$_numtimesslept $_numtimesmaybesleepcalled
...
#@_lastloadavgs;	# the values from GetCpuLoad last time we checked
...
sub maybe_sleep {	
	...
	if ($_maxload && 
	  ($t1 - $_lastloadchecktime >= 0.5)) {
		...
		my @loads = GetCpuLoad();	# (1minavg, 5minavg, 15minavg)
		@_lastloadavgs = @loads;
...
sub GetCpuLoad {
	...
	if ($loads[0] == 0) {	# either the load is that low, or Sys::CpuLoad::load()'s
							# just retuning (0,0,0) like on OSX as of 11/2004

		my $origpath = $ENV{PATH};	# save and restore PATH for taint safety
		$ENV{PATH} = "/usr/bin:/bin";
		my $success = open(UPTIME, "/usr/bin/uptime |");
		$ENV{PATH} = $origpath;
	 	return @loads unless $success;
...
Sets or gets the minimum amount time, in seconds, that maybe_sleep() will
sleep when it detects that a sleep is appropriate because the load has
gone above the value set with max_load(). 

=item over_load_sleep_drift ()

Sets or gets the 'drift' in the amount time, that maybe_sleep() will
sleep when it detects that a sleep is appropriate because the load has
gone above the value set with max_load().  If maybe_sleep() sleeps because
the 1-minute average load has exceeded max_load(), it will sleep between 
over_load_min_sleep_time()  and  
over_load_min_sleep_time() + over_load_sleep_drift() seconds.
