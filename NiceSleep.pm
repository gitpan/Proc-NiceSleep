...
# $Id: NiceSleep.pm,v 1.6 2004/08/31 20:31:14 joshr Exp $
#############################################################################
	...
	if ($_maxload && 
	  ($t1 - $_lastloadchecktime >= 0.5)) {
		...
		if ($loads[0] && $loads[0] > $_maxload) {	
			...
			$timetosleep = MAX( 
				$_over_load_minsleeptime + rand($_over_load_sleep_drift), $timetosleep, $_minsleeptime
			);	
		} 
	}
...
sub GetCpuLoad {
	...
	if ($loads[0] == 0) {	# either the load is that low, or it's 
							# just retuning (0,0,0) like on OSX
		my $out = `uptime`;	 	# use shell to search in PATH
		chomp($out);
